...
function Header() {
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'Enter' && filteredCourses.length > 0) {
      navigate(`/courses/${filteredCourses[0].slug}`);
      setSearchTerm('');
  ...
  return (
    <>
      <div className="h-16 bg-[#e6e6e6] shadow-md content-center sticky top-0 z-[9999] sm:h-16">
        <div className="justify-between px-4 flex items-center md:px-14 lg:justify-center:p-4 xl:px-21">
          <div className="Logo flex flex-row items-center gap-3">
            <img className="h-10 rounded-lg" src="./src/logo.jpg" alt="LOGO" />
            <h1 className="font-bold text-2xl xl:text-2xl">PrepVio</h1>
          </div>

          <Hamburger />

          <div className="explore-btn hidden lg:block lg:px-3 xl:block content-center xl:px-10 xl:ps-18">
            <Menu
              trigger="hover"
              openDelay={100}
              closeDelay={100}
          ...
          <div className="search-btn hidden lg:block lg:content-center relative w-78 xl:w-98 xl:text-lg">
            <input
              type="text"
              placeholder="What do you want to learn...?"
              className="border rounded-full px-6 text-lg w-full pr-10 h-[31px]"
              value={searchTerm}
              onChange={(e) => {
                setSearchTerm(e.target.value);
                setIsSuggestionOpen(true); // open suggestions on typing
              }}
              onKeyDown={handleKeyDown}
              onFocus={() => setIsSuggestionOpen(true)} // open suggestions on focus
              onBlur={() => {
                // delay closing to allow clicking links
                setTimeout(() => setIsSuggestionOpen(false), 150);
              }}
              autoComplete="off"
            />
            <svg
              ...
              strokeWidth="2.5"
              stroke="currentColor"
              className="absolute right-3 top-1/2 transform -translate-y-1/2 size-5 text-gray-500 pointer-events-none"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
            ...
            {/* Suggestion dropdown */}
            {isSuggestionOpen && filteredCourses.length > 0 && (
              <div
                style={{
                  position: 'absolute',
                ...
                onMouseLeave={() => {
                  setIsSuggestionOpen(false);
                  setSearchTerm(''); // Clear search input on mouse leave
                }}
              ...
              >
                {filteredCourses.map((course) => (
                  <Link
                    ...
                    onClick={() => {
                      setSearchTerm(''); // clear input on click
                      setIsSuggestionOpen(false); // close suggestions on click
                    }}
                    style={{
                      display: 'block',
                      padding: '8px 12px',
          ...
          <div className="language-btn hidden lg:block content-center lg:px-3 xl:px-14">
            <Menu
              trigger="hover"
              openDelay={100}
              closeDelay={100}
